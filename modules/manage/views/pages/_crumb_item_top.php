...
<tr class="crumb-rows">
    <td class="p-r-10" width="50%">
        <?= Html::activeDropDownList($model, "breads_top_arr[{$index}][page_id]", Pages::getFilterList(), [
            'class' => 'form-control select2',
            'onchange' => 'set_crumb_top(this)',
            'data-urls' => json_encode($data),
            'prompt' => ' - Страница - '
        ]) ?>
    </td>
    <td class="p-r-10">
        <?= Html::activeTextInput($model, "breads_top_arr[{$index}][name]", [
            'class' => 'form-control',
            'onkeyup' => 'set_crumb_top()',
